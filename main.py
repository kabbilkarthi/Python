    ...
    with open('hosts', 'r') as hosts:
        for h in hosts:
            ...
            try:
                ...
                print(f'/remote/kickstart/bin/ks_ansible --- MYQSC=QSC-W MYOS=AlmaLinux8.4_amd64 MYHOST={hostnames} MYNETMASK={netmask} MYOWNER={owner} MYLOCATE="{location}" MYSNPSLEVEL=prod MYMAC={nic} MYPATCHLEVEL=22P2\n')

            except subprocess.CalledProcessError as e:
                print(f"Command failed for {host}: {e}\n")
            except AttributeError as e:
                print(f"Regex matching failed for {host}: {e}\n")
